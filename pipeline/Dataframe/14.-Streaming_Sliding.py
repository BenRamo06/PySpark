...
# We create a sliding window on timeReceived field with 10 minutes of difference between each window
# Once the window is created, we create another intermediate window 5 minutes later
# We select initial,end window and count of rows between windows
sliding = windowing_df.groupBy(window("timeReceived", "10 minutes", "5 minutes")) \
...
sliding = windowing_df.select("eventId","timeReceived",window("timeReceived", "10 minutes", "5 minutes"), "window.start","window.end").orderBy("timeReceived")
